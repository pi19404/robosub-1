...
void sendCommand( uint8_t address, uint8_t m_dir, uint16_t m_speed );

const int16_t ctrl1 = 128;
const int16_t ctrl2 = 129;
const int16_t ctrl3 = 130;
const int16_t m1_ctrl = ctrl1;
const int16_t m2_ctrl = ctrl2;
const int16_t m3_ctrl = ctrl1;
const int16_t m4_ctrl = ctrl2;
const int16_t max_volts = int16_t( 22*5.12 ); // value is voltage * 5.12
const int16_t max_volt_cmd = 3;

int16_t m1_dir=0; //default in the forward direction
int16_t m2_dir=0; //default in the forward direction
int16_t m1_speed=0;
int16_t m2_speed=0;

float speed_scale_factor=0.70;

int16_t m3_dir=4; //default in the forward direction
int16_t m4_dir=4; //default in the forward direction
int16_t vertical_speed=0;
int16_t c_state=0;
int16_t z_state=0;
int16_t button_press=0;
...
void loop()
{
   ...
   //****// Control of Motors 1 and 2 //****//
       ...
       sendCommand(m1_ctrl, m1_dir, int16_t(m1_speed * speed_scale_factor));
       sendCommand(m2_ctrl, m2_dir, int16_t(m2_speed * speed_scale_factor));
   ...
   //****// Control of Motors 3 and 4 //****//   
        ...
        if (button_press==1)
        {
            ...
            sendCommand(m3_ctrl, m3_dir, int16_t(abs(vertical_speed) * speed_scale_factor));
            sendCommand(m4_ctrl, m4_dir, int16_t(abs(vertical_speed) * speed_scale_factor));
            button_press=0;
        }
