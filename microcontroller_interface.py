...
def get_lock():
    ...
    while in_sync == False:
        ...
        # otherwise, we will jump back to the top of the loop and get another
        # byte
        if current_byte == control_byte:
            ...
            # check to see if the control byte is in the proper location in the
            # received packets
    ...
    while success == False:
        ...
        if packet[0] != control_byte:  # if we are not in sync
            print ("Error: lost sync. "
                   "Press the [Enter] key to attempt to re-sync")
