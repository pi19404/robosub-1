...
namespace decision {
  TrafficLightDLM::TrafficLightDLM(movement::FuzzySets *fuzzy_sub_state,
    ...
    DEBUG_METHOD();
    m_forward_eye = new vision::TrafficLightEye(m_forward_vidcap);
    m_downward_eye = new vision::TrafficLightEye(m_downward_vidcap);
  }

  // TODO(LPE) Implement.
  bool TrafficLightDLM::can_assume_control() {
    return true;
  }

  // TODO(LPE) Implement.
  void TrafficLightDLM::activate() {
    DEBUG_METHOD();
  ...
  string can_call_traffic_light_dlm() {
    DEBUG_METHOD();
    return string("can_call_traffic_light_dlm");
  }
}  // namespace decision
