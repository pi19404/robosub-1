...
namespace decision {
  PathDLM::PathDLM(movement::FuzzySets *fuzzy_sub_state,
    ...
    DEBUG_METHOD();
    m_forward_eye = new vision::PathEye(m_forward_vidcap);
    m_downward_eye = new vision::PathEye(m_downward_vidcap);
  }

  // TODO(LPE) Implement.
  bool PathDLM::can_assume_control() {
    return true;
  }

  // TODO(LPE) Implement.
  void PathDLM::activate() {
    DEBUG_METHOD();
    set_mission_accomplished(false);
    return;
  }
  ...
  string can_call_path_dlm() {
    DEBUG_METHOD();
    return string("can_call_path_dlm");
  }
}  // namespace decision
