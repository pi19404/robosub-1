...
namespace decision {
  ParkingDLM::ParkingDLM(movement::FuzzySets *fuzzy_sub_state,
    ...
    DEBUG_METHOD();
    m_forward_eye = new vision::ParkingEye(m_forward_vidcap);
    m_downward_eye = new vision::ParkingEye(m_downward_vidcap);
  }

  // TODO(LPE) Implement.
  bool ParkingDLM::can_assume_control() {
    return true;
  }

  // TODO(LPE) Implement.
  void ParkingDLM::activate() {
    DEBUG_METHOD();
  ...
  string can_call_parking_dlm() {
    DEBUG_METHOD();
    return string("can_call_parking_dlm");
  }
}  // namespace decision
