...
namespace decision {
  PizzaDLM::PizzaDLM(movement::FuzzySets *fuzzy_sub_state,
    ...
    DEBUG_METHOD();
    m_forward_eye = new vision::PizzaEye(m_forward_vidcap);
    m_downward_eye = new vision::PizzaEye(m_downward_vidcap);
  }

  // TODO(LPE) Implement.
  bool PizzaDLM::can_assume_control() {
    return true;
  }

  // TODO(LPE) Implement.
  void PizzaDLM::activate() {
    DEBUG_METHOD();
  ...
  string can_call_pizza_dlm() {
    DEBUG_METHOD();
    return string("can_call_pizza_dlm");
  }
}  // namespace decision
