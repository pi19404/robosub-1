...
class Arduino(object):
    ...
    def read(self, s):
        assert( len(s) == 4 )
        armed, heading, depth = struct.unpack('>BHB', s)
        if armed != 0:
            self.armed.set()
            self.disarmed.clear()
        else:
    ...
    def go(self):
        ...
        def work():
            ...
            while self.work:
                ...
                self.ser.write( self.command_string(status, self.x, self.y, self.z, self.delta_heading) )
